...
from augmented_partition.model.network import SO2Net
from augmented_partition.model.utils import create_rotation_matrix, rotate_irrep_vector


def create_toy_data(input_vector):
    x = torch.tensor([1, 1])
    edge_distance_vec = torch.tensor(
        [[0.0, 0.0, 0.0], input_vector], dtype=torch.float32
    )  # shape = (2,3) = [2, 3]
    ...
    for i in range(len(edge_index[0])):
        distance_vector = (
            edge_distance_vec[edge_index[1][i]] - edge_distance_vec[edge_index[0][i]]
...
def test_equivariance():
    ...
    )  # create rotation matrix in cartesian space
    ...
    assert torch.allclose(node_error, torch.zeros_like(node_error), atol=1e-5), (
        "Node equivariance test failed!"
    )
    assert torch.allclose(edge_error, torch.zeros_like(edge_error), atol=1e-5), (
        "Edge equivariance test failed!"
    )
