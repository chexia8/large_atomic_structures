...
from augmented_partition.lib_equiformer.layer_norm import get_normalization_layer
from augmented_partition.lib_equiformer.radial_function import RadialFunction
from augmented_partition.lib_equiformer.SO2_operations import SO2_Convolution
from augmented_partition.lib_equiformer.SO3 import SO3_Embedding, SO3_LinearV2
...
class SO2NodeUpdate(torch.nn.Module):
    ...
    def forward(self, x, atomic_numbers, edge_distance, edge_index, edge_fea):
...
class SO2NodeUpdate_local(torch.nn.Module):
    ...
    def forward(self, x, atomic_numbers, edge_distance, edge_index, hidden_fea):
        # Compute edge scalar features (invariant to rotations)
        # Uses atomic numbers and edge distance as inputs
        if self.use_atom_edge_embedding:
            source_element = atomic_numbers[edge_index[0]]  # Source atom atomic number
...
class SO2HiddenUpdate(
    torch.nn.Module
):  # creates hidden features for many nody representations
    def __init__(
        self,
        sphere_channels,
        hidden_channels,
