...
from augmented_partition.lib_equiformer.SO3 import SO3_Embedding, SO3_Rotation
from augmented_partition.model.transformer_block import EdgeBlockV2, NodeBlockV2
...
class SO2Net_local(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        node_embedding = SO3_Embedding(
            num_subgraph_nodes, self.lmax, self.sphere_channels, device, dtype
        )  # first dimension is the number of atoms, second dimension is the number of coefficients, third dimension is the number of channels
        edge_embedding = SO3_Embedding(
            num_subgraph_edges, self.lmax, self.sphere_channels, device, dtype
        )  # first dimension is the number of edges, second dimension is the number of coefficients,
        ...
        hidden_edge_embedding = (
            edge_embedding  # hidden edge embedding is initialized to the edge embedding
        )
...
def convert_to_irreps(inp, output_channels, lmax, lin_node):
    """
