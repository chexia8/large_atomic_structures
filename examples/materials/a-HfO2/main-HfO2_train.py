...
from augmented_partition.lib_equiformer import SO3, process_irreps
from augmented_partition.model import data, network, structure, training
...
def main():
    ...
    orbital_types = [
        [0, 1],
        [0, 0, 1, 2],
    ]  # must be in ascending order of atomic numbers                                                 # basis rank of each atom in the structure
    ...
    if partitioning == "slice_center":
        slice_list = [
            ...
            1200,
            1400,
        ]  # slice boundaries for partitioning the structure into subgraphs
        cutoff = 1.5
        data_loader = data.batch_data_subgraph(
            a_HfO2,
            slice_list,
            cutoff,
        ...
        print(
            "Data loader created - using " + str(len(slice_list)) + " slices",
            flush=True,
        )
        ...
        data_loader = data.batch_data_graphpartition(
            a_HfO2,
            num_subgraph,
            num_batch,
            equivariant_blocks=equivariant_blocks,
        ...
        print(
            "Data loader created - using " + str(num_subgraph) + " subgraphs",
            flush=True,
        )
    if dist.is_initialized():
        dist.barrier()
    ...
    # use with a restarted model, to test the model
        ...
        test_data = torch.load(
            "test_data_structures/model_HfO2x2_structure_1_training_1500_2.pt"
        )
        test_data_loader = data.batch_data_load(
            test_data,
            equivariant_blocks,
            out_slices,
            construct_kernel,
